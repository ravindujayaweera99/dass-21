/* Vanilla JS single-page survey runner (DASS-21) mirroring the Shiny flow.
   ...
   UPDATE: Questions are now REQUIRED per page (cannot proceed unless all visible Qs are answered). */
...
(function () {
  ...
  function validatePage(pageNum) {
    const { startIdx, endIdx } = getPageRange(pageNum);
    const missing = [];
    for (let i = startIdx; i < endIdx; i++) {
      if (state.responses[i] === null) missing.push(i);
    }
    return missing; // 0-based indices
  }

  function clearValidationUI() {
    const err = document.querySelector("#page .validation-error");
    if (err) err.remove();

    document.querySelectorAll("#page .well.missing").forEach((el) => {
  ...
  function showValidationUI(missingIdxs) {
    clearValidationUI();

    const box = document.querySelector("#page .content-box");
    if (!box) return;
    ...
    err.setAttribute(
      "style",
      "margin:0 0 15px 0; padding:12px 14px; border-radius:10px; background:#fff3cd; color:#664d03; border:1px solid #ffe69c; font-size:14px;"
    );
    err.textContent = cfg["error-msg"] || "Please answer all questions on this page before continuing.";
    box.insertBefore(err, box.firstChild);

    let firstWell = null;
    for (const i of missingIdxs) {
      const well = document.querySelector(`#qwell_${i + 1}`);
      if (well) {
        well.classList.add("missing");
        well.style.outline = "2px solid #b30059";
        well.style.outlineOffset = "3px";
        if (!firstWell) firstWell = well;
      }
    }

    if (firstWell) firstWell.scrollIntoView({ behavior: "smooth", block: "start" });
  }
  ...
  function renderIntro0() {
    ...
    ack.setAttribute(
      "style",
      "max-width: 800px; margin: 20px auto; text-align:center; font-size:16px; color:#333;"
    );
    ...
    footer.setAttribute(
      "style",
      "display:flex; align-items:center; justify-content:center; gap:15px; margin-top:40px;"
    );
  ...
  function renderIntro1() {
    ...
    nav.appendChild(
      button(cfg.page1.prevLabel, () => {
        state.page = 0;
        render();
      })
    );
    nav.appendChild(
      button(cfg.page1.nextLabel, () => {
        state.page = 2;
        render();
      })
    );
  ...
  function renderQuestionsPage(pageNum) {
    ...
    for (let i = startIdx; i < endIdx; i++) {
      ...
      well.className = "well";
      well.id = `qwell_${i + 1}`; // used for required highlighting
      ...
      for (const opt of cfg.respChoices) {
        ...
        input.addEventListener("change", () => {
          ...
          // remove error once user starts fixing
          clearValidationUI();
        });
    ...
    // Prev
    nav.appendChild(
      button(cfg.results.prevLabel, () => {
        clearValidationUI();
        state.page = Math.max(0, pageNum - 1);
        render();
      })
    );
    ...
    nav.appendChild(
      button(cfg.page0.nextLabel, () => {
        const missing = validatePage(pageNum);
        if (missing.length > 0) {
          showValidationUI(missing);
          return;
        }
        clearValidationUI();
  ...
  function renderResults() {
    ...
    for (const r of rows) {
      ...
      tdScale.setAttribute(
        "style",
        `padding:10px; border-bottom:1px solid #ddd; background:${bg}; color:${fg}; text-align:left;`
      );
      ...
      tdRaw.setAttribute(
        "style",
        `text-align:center; padding:10px; border-bottom:1px solid #ddd; background:${bg}; color:${fg};`
      );
    ...
    note.setAttribute(
      "style",
      "margin-top:20px; padding:15px; background-color:#f9f9f9; border-radius:10px; display:flex; align-items:flex-start; gap:10px;"
    );
    ...
    nav.appendChild(
      button(cfg.results.finishLabel, () => {
        reset();
        render();
      })
    );
    nav.appendChild(
      button(cfg.results.pdfLabel, () => {
        window.print();
      })
    );
    box.appendChild(nav);
